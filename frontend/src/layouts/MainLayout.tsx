...
export default function MainLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-gray-100 font-sans">
      ...
      {/* Sidebar */}
      <aside className={`bg-indigo-600 text-white flex flex-col transition-all duration-300 shadow-2xl z-20 ${open ? "w-64 p-5" : "w-20 p-4"}`}>
        {/* Toggle Button */}
        <button 
        ...
        {/* LOGO AREA: Now converted to a Button/Link */}
        <Link 
          to="/" 
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white shadow-sm p-4 px-8 flex justify-between items-center z-10 border-b border-gray-100">
          ...
          <div className="flex items-center gap-4">
            <button 
              ...
              className="bg-indigo-600 text-white px-6 py-3 rounded-2xl hover:bg-indigo-700 transition-all flex items-center gap-2 font-black shadow-xl shadow-indigo-100 text-sm active:scale-95"
            >
        ...
        <main className="p-6 overflow-auto flex-1 bg-[#f8fafc]">
          {children}
        </main>
      </div>
