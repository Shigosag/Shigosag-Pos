...
import { getSocketUrl } from "../api/api"; // Helper for dynamic backend URL
...
export default function Dashboard() {
  ...
  useEffect(() => {
    // Uses dynamic URL based on environment (Prod vs Dev)
  ...
  return (
    <div className="space-y-6 pb-12 max-w-7xl mx-auto">
      ...
      <div className="bg-indigo-600 text-white p-5 rounded-[1.5rem] flex justify-between items-center shadow-xl shadow-indigo-100">
        <div>
          <h3 className="font-bold text-sm">System Status</h3>
          <p className="text-[11px] opacity-90 font-medium">Global networks stable</p>
        </div>
        <div className="text-sm font-bold flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-full">
      ...
      {/* BALANCE CARD */}
      <div className="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-md flex justify-between items-center group hover:border-indigo-200 transition-all">
        <div>
          <p className="text-gray-400 text-[11px] font-black uppercase tracking-[0.2em] mb-2">Available Balance</p>
          <p className="text-5xl font-black text-emerald-600 tracking-tighter">{format(stats.balance)}</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {cards.map((card) => (
          <Link 
            key={card.title} 
            to={card.path} 
            className={`p-6 rounded-[2rem] shadow-sm hover:shadow-xl transform transition-all duration-300 hover:-translate-y-2 ${
              coloredMode 
