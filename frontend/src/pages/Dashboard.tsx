...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6 pb-10 font-sans">
      ...
      <div className="bg-indigo-600 text-white p-4 rounded-xl flex justify-between items-center hover:shadow-xl transition-all duration-300 transform hover:-translate-y-0.5">
        <div>
        ...
        {cards.map((card) => (
          <Link
            ...
            className={`p-5 rounded-xl shadow-sm hover:shadow-xl transform transition-all duration-300 hover:-translate-y-1 ${
              coloredMode ? `bg-gradient-to-r ${card.color} text-white` : "bg-white text-gray-800 border border-gray-200"
            }`}
          >
        ...
        <div className="h-[250px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              ...
              <Line type="monotone" dataKey="sales" stroke="#ef4444" strokeWidth={4} dot={{ r: 5, fill: '#ef4444', strokeWidth: 2, stroke: '#fff' }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
