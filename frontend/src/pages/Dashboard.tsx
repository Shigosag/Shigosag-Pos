...
export default function Dashboard() {
  ...
  // Formatting with commas and NGN symbol
  ...
  return (
    <div className="space-y-6 pb-12 max-w-7xl mx-auto animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-center mb-4">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight flex items-center gap-3">
             <Rocket className="text-indigo-600" size={32} /> Shigosag POS
          </h1>
          <p className="text-gray-500 mt-1 font-medium italic">Real-time fintech dashboard system</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`relative w-14 h-8 rounded-full transition-all duration-300 ${
              coloredMode ? "bg-indigo-600" : "bg-gray-300"
            }`}
          >
            <div
      ...
      <div className="bg-indigo-600 text-white p-5 rounded-[1.5rem] flex justify-between items-center shadow-lg mb-6">
        <div>
          <h3 className="font-bold text-sm">System Status</h3>
          <p className="text-[11px] opacity-90 font-medium">Global networks stable - All services operational</p>
        </div>
      ...
      {/* AVAILABLE BALANCE */}
      <div className="bg-white p-10 rounded-[1.5rem] border border-gray-100 shadow-md flex justify-between items-center group hover:border-indigo-200 transition-all mb-4">
        <div>
        ...
        <div className="p-5 bg-indigo-50 text-indigo-600 rounded-2xl shadow-inner">
           <Wallet size={40} />
        </div>
      </div>

      {/* QUICK ACTIONS GRID */}
      <h2 className="text-xl font-bold text-gray-800 ml-2 mb-4">Quick Actions</h2>
      ...
      {/* SALES CHART */}
      <div className="bg-white p-8 rounded-[1.5rem] border border-gray-100 shadow-sm mt-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
        {[
          ...
          <div key={i} className="bg-white p-6 rounded-[1.5rem] shadow-sm border border-gray-100 hover:shadow-lg transition-all transform hover:-translate-y-1">
      ...
      {/* LIVE SALES FEED */}
      <div className="bg-white rounded-[1.5rem] shadow-sm border border-gray-100 p-8 mt-10">
        <h2 className="text-xl font-black text-gray-800 mb-6 uppercase text-xs tracking-[0.2em]">Live Sales Activity</h2>
        <div className="space-y-4 max-h-64 overflow-y-auto pr-2">
          <div className="p-4 bg-slate-50 rounded-xl flex items-center gap-3 text-sm font-bold text-gray-600 border border-slate-100 animate-pulse">
            <div className="w-2 h-2 bg-indigo-500 rounded-full" /> 🛒 New retail sale completed
          </div>
          <div className="p-4 bg-slate-50 rounded-xl flex items-center gap-3 text-sm font-bold text-gray-600 border border-slate-100">
      ...
      <div className="grid md:grid-cols-2 gap-6 mt-8">
        <div className="bg-white p-8 rounded-[1.5rem] shadow-sm border border-gray-100">
          ...
          <div className="space-y-4 text-sm">
            {liveFeed.length === 0 ? (
              <p className="text-gray-300 font-bold italic py-4 text-center">Waiting for terminal activity...</p>
            ) : (
              liveFeed.map((tx, i) => (
                <div key={i} className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border-l-4 border-emerald-500 font-bold">
        ...
        <div className="bg-white p-8 rounded-[1.5rem] shadow-sm border border-gray-100 flex flex-col justify-between">
          ...
          <Link 
            to="/history" 
            className="mt-8 bg-black text-white text-center py-5 rounded-xl font-black hover:bg-gray-800 transition-all shadow-xl shadow-gray-200"
          >
